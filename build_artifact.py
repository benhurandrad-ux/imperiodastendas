...
# assets
s=re.sub(r'(src|href)="(assets/[^"]+)"',lambda m:'%s="%s"'%(m.group(1),data_uri(os.path.join(here,m.group(2)))),s)
...
s=re.sub(r'<title>[^<]*</title>','<title>Império das Tendas</title>',s,count=1)
